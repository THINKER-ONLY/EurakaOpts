...
def native_sparse_attention(
    ...
    # Preserve the block id for copy bounds while reusing the scaled start.
    cache_block_start = (
        S == 1
        and (
            (
                BS == 16
                and (
                    (dim == 32 and seq_len >= 2048)
                    or (dim == 64 and seq_len == 4096)
                    or dim == 128
                )
            )
            or (BS == 32 and dim == 128)
        )
    )
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            if cache_block_start:
                block_id = T.alloc_local([1], T.int32)
                block_start = T.alloc_local([1], T.int32)
            ...
            if cache_block_start:
                block_id[0] = BlockIndices[i_b, i_t, i_h, 0]
                block_start[0] = block_id[0] * BS
            if S > 1:
                T.fill(acc_o, 0)
