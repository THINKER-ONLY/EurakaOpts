    ...
    def main_split(
        ...
        with T.Kernel(
            ...
            T.copy(
                Q[
                    bid,
                    head_tile * valid_block_h : (head_tile + 1) * valid_block_h,
                    :,
                ],
                Q_shared,
            )
        ...
        with T.Kernel(
            heads * output_parts, batch, threads=128
        ) as (pid, bz):
            hid = pid // output_parts
            output_part = pid % output_parts
            po_local = T.alloc_fragment([output_dim], dtype)
            o_accum_local = T.alloc_fragment([output_dim], accum_dtype)
            ...
            for k in T.serial(num_split):
                for i in T.Parallel(output_dim):
                    po_local[i] = output_partial[
                        bz,
                        hid,
                        k,
                        output_part * output_dim + i,
                    ]
                lse_local_split = glse[bz, hid, k]
            ...
            for i in T.Parallel(output_dim):
                Output[
                    bz, hid, output_part * output_dim + i
                ] = o_accum_local[i]
