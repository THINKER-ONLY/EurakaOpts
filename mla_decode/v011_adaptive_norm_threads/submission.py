        ...
        if materialize_scores:
            ...
            with T.Kernel(
                ...
                for k in T.Pipelined(loop_range, num_stages=0):
                    ...
                    T.copy(
                        KV[bid, kv_start:kv_end, cur_kv_head, 0:output_dim],
                        V_shared,
                    )
                    T.gemm(
                        acc_s_cast,
            ...
            with T.Kernel(
                ...
                batch,
                threads=(
                    64 if seqlen_kv // num_split <= 256 else 128
                ),
            ) as (norm_pid, norm_bid):
                Score_shared = T.alloc_shared([block_h, block_n], dtype)
                acc_s = T.alloc_fragment([block_h, block_n], accum_dtype)
                norm_head_tile = norm_pid // num_split
                norm_split = norm_pid % num_split

                loop_range = T.ceildiv(seqlen_kv // num_split, block_n)
                for k in T.Pipelined(loop_range, num_stages=0):
                    kv_start = (
                        (seqlen_kv // num_split) * norm_split + k * block_n
                    )
                    kv_end = (
                        (seqlen_kv // num_split) * norm_split
                        + (k + 1) * block_n
                    )
                    T.copy(
                        score_partial[
                            norm_bid,
                            norm_head_tile
                            * valid_block_h : (norm_head_tile + 1)
                            * valid_block_h,
                            kv_start:kv_end,
                        ],
                        Score_shared,
                    ...
                    for i, j in T.Parallel(block_h, block_n):
                        acc_s[i, j] = T.exp2(
                            ...
                            - glse[
                                norm_bid,
                                norm_head_tile * valid_block_h + i,
                                norm_split,
                            ]
                        )
                    T.copy(acc_s, Score_shared)
