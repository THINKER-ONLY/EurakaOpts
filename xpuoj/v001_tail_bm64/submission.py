...
def _moe_forward_kernel(
    ...
    threads = 256
    num_stages = 1
    tail_splits = routing_block_m // tail_block_m
    ...
    def kernel(
        ...
        # FC1 full blocks retain the official 128x128 schedule unchanged.
        with T.Kernel(num_blocks_m, T.ceildiv(intermediate, block_n), threads=threads) as (bx, by):
            input_shared = T.alloc_fragment((routing_block_m, block_k), dtype=dtype)
            gate_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            up_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            gate_local = T.alloc_fragment((routing_block_m, block_n), dtype=accum_dtype)
            up_local = T.alloc_fragment((routing_block_m, block_n), dtype=accum_dtype)
            ...
            actual_rows = T.max(0, T.min(routing_block_m, group_size - token_offset))

            if actual_rows == routing_block_m:
                T.clear(gate_local)
                T.clear(up_local)

                for k in T.Pipelined(T.ceildiv(hidden, block_k), num_stages=num_stages):
                    T.copy(
                ...
                for i, j in T.Parallel(routing_block_m, block_n):
                ...
                for i, j in T.Parallel(routing_block_m, block_n):
                    up_logits[block_start + i, by * block_n + j] = up_local[i, j]

        # FC1 tail blocks use two 64-row sub-blocks per routed 128-row block.
        with T.Kernel(
            num_blocks_m * tail_splits,
            T.ceildiv(intermediate, block_n),
            threads=threads,
        ) as (bx, by):
            input_shared = T.alloc_fragment((tail_block_m, block_k), dtype=dtype)
            gate_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            up_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            gate_local = T.alloc_fragment((tail_block_m, block_n), dtype=accum_dtype)
            up_local = T.alloc_fragment((tail_block_m, block_n), dtype=accum_dtype)

            T.use_swizzle(10)

            parent_bx = bx // tail_splits
            tail_id = bx % tail_splits
            expert_id = group_idx_for_bx[parent_bx]
            parent_start = parent_bx * routing_block_m
            block_start = parent_start + tail_id * tail_block_m
            group_size = group_sizes[expert_id]
            padded_start = group_padded_offsets[expert_id]
            parent_token_offset = parent_start - padded_start
            token_offset = block_start - padded_start
            actual_rows = T.max(
                0,
                T.min(routing_block_m, group_size - parent_token_offset),
            )
            tail_rows = T.max(0, T.min(tail_block_m, group_size - token_offset))
            ...
            if actual_rows < routing_block_m:
                if tail_rows > 0:
                    ...
                    for k in T.Pipelined(T.ceildiv(hidden, block_k), num_stages=num_stages):
                        T.copy(
                            stacked_expert_tokens[
                                block_start : block_start + tail_block_m,
                                k * block_k : (k + 1) * block_k,
                            ],
                            input_shared,
                        )
                    ...
                    for i, j in T.Parallel(tail_block_m, block_n):
                        gate_local[i, j] = gate_local[i, j] * (
                            1.0 / (1.0 + T.exp2(-gate_local[i, j] * scale))
                        )
                        up_local[i, j] = up_local[i, j] * gate_local[i, j]
        ...
        # FC2 full blocks retain the official 128x128 schedule unchanged.
        with T.Kernel(num_blocks_m, T.ceildiv(hidden, block_n), threads=threads) as (bx, by):
            up_shared = T.alloc_fragment((routing_block_m, block_k), dtype=dtype)
            down_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            out_local = T.alloc_fragment((routing_block_m, block_n), dtype=accum_dtype)
            ...
            if actual_rows == routing_block_m:
                ...
                for k in T.Pipelined(T.ceildiv(intermediate, block_k), num_stages=num_stages):
                    T.copy(
                        up_logits[
                            block_start : block_start + routing_block_m,
                            k * block_k : (k + 1) * block_k,
                ...
                for i, j in T.Parallel(routing_block_m, block_n):
                    out[block_start + i, by * block_n + j] = (
                        out_local[i, j]
                        * routed_expert_weights[raw_start + token_offset + i]
                    )

        # FC2 mirrors the FC1 tail mapping so it only reads initialized rows.
        with T.Kernel(
            num_blocks_m * tail_splits,
            T.ceildiv(hidden, block_n),
            threads=threads,
        ) as (bx, by):
            up_shared = T.alloc_fragment((tail_block_m, block_k), dtype=dtype)
            down_shared = T.alloc_shared((block_n, block_k), dtype=dtype)
            out_local = T.alloc_fragment((tail_block_m, block_n), dtype=accum_dtype)
