...
def _moe_forward_kernel(
    ...
    def kernel(
        ...
        with T.Kernel(num_blocks_m, T.ceildiv(intermediate, fc1_block_n), threads=threads) as (bx, by):
            ...
            weight_shared = T.alloc_shared((fc1_block_n, block_k), dtype=dtype)
            ...
            for k in T.serial(T.ceildiv(hidden, block_k)):
                T.copy(
                ...
                T.gemm(input_shared, weight_shared, up_local, transpose_B=True)
                T.sync_threads()
            ...
            for i, j in T.Parallel(routing_block_m, fc1_block_n):
                if i < actual_rows:
                    ...
                    up_logits[block_start + i, by * fc1_block_n + j] = (
