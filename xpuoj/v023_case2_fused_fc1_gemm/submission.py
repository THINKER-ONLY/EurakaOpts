...
def _moe_forward_kernel_fused(
    ...
    threads = 256
    fused_fc1_threads = 256
    num_stages = 1
    fused_fc1_num_stages = 1
    ...
    def kernel(
        ...
        with T.Kernel(num_blocks_m, T.ceildiv(intermediate, fc1_block_n), threads=fused_fc1_threads) as (bx, by):
            input_shared = T.alloc_fragment((routing_block_m, fc1_block_k), dtype=dtype)
            gate_up_shared = T.alloc_shared((fc1_block_n * 2, fc1_block_k), dtype=dtype)
