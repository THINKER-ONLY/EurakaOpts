...
def _moe_forward_kernel(
    ...
    block_k = 128
    fc1_block_n = 128
    fc2_block_n = 256 if hidden == 7168 and intermediate == 2048 else 128
    threads = 256
    num_stages = 1
    ...
    def kernel(
        ...
        with T.Kernel(num_blocks_m, T.ceildiv(intermediate, fc1_block_n), threads=threads) as (bx, by):
            input_shared = T.alloc_fragment((routing_block_m, block_k), dtype=dtype)
            gate_shared = T.alloc_shared((fc1_block_n, block_k), dtype=dtype)
            up_shared = T.alloc_shared((fc1_block_n, block_k), dtype=dtype)
