...
def _unpack_experts_kernel(
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            padded_start = group_padded_offsets[expert_id]
            for i, j in T.Parallel(expert_block_m, block_n):
                if (num_experts != 16 or group_size < 128) and i < group_size:
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            for i, j in T.Parallel(expert_block_m, block_n):
                ...
                if num_experts == 16 and i >= group_size:
...
def _e32_fc1_tail_kernel(
    ...
    def kernel(
        stacked_expert_tokens: T.Tensor(
            (total_padded_tokens, hidden), T.float16
        ),
        combined_w: T.Tensor(
            (num_experts, output_width, hidden), T.float16
        ),
        group_sizes: T.Tensor((num_experts,), T.int32),
        group_padded_offsets: T.Tensor((num_experts + 1,), T.int32),
        gate_up_output: T.Tensor(
            (num_experts, expert_block_m, output_width), T.float16
        ),
    ):
        with T.Kernel(
            T.ceildiv(output_width, block_n),
            threads=threads,
        ) as bn:
            input_shared = T.alloc_shared((block_m, block_k), T.float16)
            weight_shared = T.alloc_shared((block_n, block_k), T.float16)
            accum = T.alloc_fragment((block_m, block_n), T.float32)
            for expert in T.serial(expert_chunk):
                group_size = group_sizes[expert]
                padded_start = group_padded_offsets[expert]
                if group_size > tail_start:
                    T.clear(accum)
                    for bk in T.Pipelined(hidden // block_k, num_stages=1):
                        T.copy(
                            stacked_expert_tokens[
                                padded_start
                                + tail_start : padded_start
                                + tail_start
                                + block_m,
...
def _prepare_e16_pointer_tables_kernel(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
...
__device__ __forceinline__ void prepare_e16_pointer_tables_raw(
    const half_t* stacked_expert_tokens,
    const int32_t* group_sizes,
    const int32_t* group_padded_offsets,
    const half_t* gate_w,
    const half_t* up_w,
    half_t* packed_input,
    const half_t* gate_output,
    const half_t* up_output,
    const half_t* activation,
    const half_t* down_w_t,
    const half_t* down_output,
    half_t* out,
    int64_t* fc1_pointer_table,
    int64_t* down_pointer_table,
    int num_experts,
    int expert_block_m,
    int hidden,
    int intermediate) {
  int tid = static_cast<int>(threadIdx.x);
  int bn = static_cast<int>(blockIdx.x);
  int last_expert = num_experts - 1;
  int last_size = group_sizes[last_expert];
  if (bn == 0 && tid < num_experts) {
    int expert = tid;
    int64_t input_offset =
        static_cast<int64_t>(group_padded_offsets[expert]) * hidden * 2;
    if (expert == last_expert && last_size < 128) {
      input_offset = static_cast<int64_t>(expert) * expert_block_m * hidden * 2;
      fc1_pointer_table[expert] =
          reinterpret_cast<int64_t>(packed_input) + input_offset;
    } else {
      fc1_pointer_table[expert] =
          reinterpret_cast<int64_t>(stacked_expert_tokens) + input_offset;
    }
    int64_t weight_offset =
        static_cast<int64_t>(expert) * intermediate * hidden * 2;
    int64_t output_offset =
        static_cast<int64_t>(expert) * expert_block_m * intermediate * 2;
    fc1_pointer_table[num_experts + expert] =
        reinterpret_cast<int64_t>(gate_w) + weight_offset;
    fc1_pointer_table[2 * num_experts + expert] =
        reinterpret_cast<int64_t>(gate_output) + output_offset;
    fc1_pointer_table[3 * num_experts + expert] =
        reinterpret_cast<int64_t>(up_w) + weight_offset;
    fc1_pointer_table[4 * num_experts + expert] =
        reinterpret_cast<int64_t>(up_output) + output_offset;

    down_pointer_table[expert] = reinterpret_cast<int64_t>(activation)
        + static_cast<int64_t>(expert) * expert_block_m * intermediate * 2;
    down_pointer_table[num_experts + expert] =
        reinterpret_cast<int64_t>(down_w_t) + weight_offset;
    down_pointer_table[2 * num_experts + expert] =
  ...
  if (last_size < 128) {
    int col = bn * 256 + tid;
    if (col < hidden) {
      int padded_start = group_padded_offsets[last_expert];
      for (int row = 0; row < last_size; ++row) {
        packed_input[
            (static_cast<int64_t>(last_expert) * expert_block_m + row) * hidden
            + col] = stacked_expert_tokens[
                (static_cast<int64_t>(padded_start) + row) * hidden + col];
      }
    }
  }
}
"""
    ...
    def kernel(
        ...
        down_w_t: T.Tensor((num_experts, intermediate, hidden), T.float16),
        down_output: T.Tensor(
            (num_experts, expert_block_m, hidden),
            T.float16,
        ),
        out: T.Tensor((total_padded_tokens, hidden), T.float16),
        fc1_pointer_table: T.Tensor((5, num_experts), T.int64),
        down_pointer_table: T.Tensor((3, num_experts), T.int64),
    ):
        with T.Kernel(T.ceildiv(hidden, 256), threads=256) as _:
            T.import_source(source)
            T.call_extern(
                "handle",
                "prepare_e16_pointer_tables_raw",
                stacked_expert_tokens.data,
                group_sizes.data,
                ...
                T.access_ptr(
                    fc1_pointer_table[0, 0], "w", 5 * num_experts
                ),
                T.access_ptr(
                    down_pointer_table[0, 0], "w", 3 * num_experts
                ),
                num_experts,
                expert_block_m,
                hidden,
...
def _prepare_e3264_pointer_tables_kernel(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
    ...
    source = r"""\
#include <tl_templates/maca/common.h>
__device__ __forceinline__ void prepare_e3264_pointer_tables_raw(
    const half_t* stacked_expert_tokens,
    const int32_t* group_sizes,
    const int32_t* group_padded_offsets,
    const half_t* combined_w,
    half_t* packed_input,
    const half_t* gate_up_output,
    const half_t* activation,
    const half_t* down_w,
    const half_t* down_output,
    half_t* out,
  ...
  if (bn == 0 && tid < num_experts) {
    ...
    fc1_pointer_table[num_experts + expert] =
        reinterpret_cast<int64_t>(combined_w)
        + static_cast<int64_t>(expert) * intermediate * 2 * hidden * 2;
    fc1_pointer_table[2 * num_experts + expert] =
        reinterpret_cast<int64_t>(gate_up_output)
        + static_cast<int64_t>(expert) * expert_block_m * intermediate * 2 * 2;

    down_pointer_table[expert] = reinterpret_cast<int64_t>(activation)
        + static_cast<int64_t>(expert) * expert_block_m * intermediate * 2;
    down_pointer_table[num_experts + expert] = reinterpret_cast<int64_t>(down_w)
        + static_cast<int64_t>(expert) * hidden * intermediate * 2;
    down_pointer_table[2 * num_experts + expert] =
        group_sizes[expert] >= 128
        ? reinterpret_cast<int64_t>(out)
...
"""
    ...
    def kernel(
        ...
        group_padded_offsets: T.Tensor((num_experts + 1,), T.int32),
        combined_w: T.Tensor(
            (num_experts, intermediate * 2, hidden), T.float16
        ),
        packed_input: T.Tensor(
            (num_experts, expert_block_m, hidden), T.float16
        ),
        gate_up_output: T.Tensor(
            (num_experts, expert_block_m, intermediate * 2), T.float16
        ),
        activation: T.Tensor(
            (num_experts, expert_block_m, intermediate), T.float16
        ),
        down_w: T.Tensor(
            (num_experts, hidden, intermediate), T.float16
        ),
        down_output: T.Tensor(
            (num_experts, expert_block_m, hidden), T.float16
        ),
        out: T.Tensor((total_padded_tokens, hidden), T.float16),
        fc1_pointer_table: T.Tensor((3, num_experts), T.int64),
        down_pointer_table: T.Tensor((3, num_experts), T.int64),
    ):
        with T.Kernel(T.ceildiv(hidden, block_n), threads=256) as _:
            T.import_source(source)
            T.call_extern(
                "handle",
                "prepare_e3264_pointer_tables_raw",
                stacked_expert_tokens.data,
                group_sizes.data,
                group_padded_offsets.data,
                combined_w.data,
                ...
                T.access_ptr(out[0, 0], "w", total_padded_tokens * hidden),
                T.access_ptr(
                    fc1_pointer_table[0, 0], "w", 3 * num_experts
                ),
                T.access_ptr(
                    down_pointer_table[0, 0], "w", 3 * num_experts
                ),
...
def _get_kernels(
    ...
    if kernels is None:
        ...
        kernels = (
            ...
            (
                _prepare_e16_pointer_tables_kernel(
                    hidden,
                    intermediate,
                    num_experts,
                    total_padded_tokens,
            ...
            (
                _prepare_e3264_pointer_tables_kernel(
                    hidden,
                    intermediate,
                    num_experts,
                    total_padded_tokens,
...
def _get_workspace(
    ...
    if workspace is None:
        ...
        gate_up_output = (
            torch.empty(
                (
                    num_experts,
                    expert_block_m,
                    intermediate if num_experts == 16 else intermediate * 2,
                ),
                device=stacked_expert_tokens.device,
                dtype=stacked_expert_tokens.dtype,
            )
        ...
        pointer_table = (
            torch.empty(
                (5 if num_experts == 16 else 3, num_experts),
                device=stacked_expert_tokens.device,
                dtype=torch.int64,
            )
            if num_experts in (16, 32, 64)
...
def _pointer_bmm_e16_fc1(
    ...
    def table_row(row):
        address = base + row * expert_count_total * pointer_size
        return ctypes.cast(
            ctypes.c_void_p(address),
            ctypes.POINTER(ctypes.c_void_p),
        )
    ...
    status = lib.mcblasHgemmBatched(
        ...
        hidden,
        ctypes.byref(_MCBLAS_ALPHA),
        table_row(weight_row),
        hidden,
        table_row(0),
        hidden,
        ctypes.byref(_MCBLAS_BETA),
        table_row(output_row),
        intermediate,
        expert_count,
    )
    if status:
        raise RuntimeError(f"mcblasHgemmBatched E16 FC1 failed: {status}")
...
def _pointer_bmm_e16_down(
    ...
    if status:
        raise RuntimeError(f"mcblasHgemmBatched E16 down failed: {status}")
...
def run_kernel(
    ...
    (
        ...
        prepare_e16_pointer_tables,
        prepare_e3264_pointer_tables,
        e32_fc1_tail,
    ) = _get_kernels(
        hidden,
        intermediate,
    ...
    if num_experts not in (16, 32, 64):
        pack(
            stacked_expert_tokens,
            group_sizes,
            group_padded_offsets,
    ...
    if num_experts == 16:
        down_t = _get_transposed_down_weight(down_w, transpose_down)
        prepare_e16_pointer_tables(
            stacked_expert_tokens,
            group_sizes,
            group_padded_offsets,
    ...
    if num_experts != 16:
        down_t = down_w.transpose(1, 2)
    expert_chunk = 16
    if num_experts == 16:
        current_stream = torch.cuda.current_stream(device=activation.device)
        _pointer_bmm_e16_down(
            down_pointer_table,
            current_stream,
            num_experts,
            hidden,
        ...
        for stream_index, stream in enumerate(down_streams):
            with torch.cuda.stream(stream):
                ...
                _pointer_bmm_fc1(
                    ...
                    (
                        158
                        if num_experts == 32 and stream_expert_start == 0
                        else fc1_block_m
                    ),
                )
                if num_experts == 32 and stream_expert_start == 0:
