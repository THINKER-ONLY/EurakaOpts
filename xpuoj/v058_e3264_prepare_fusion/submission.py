...
def _build_e16_fc1_pointer_table_kernel(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
...
__device__ __forceinline__ void build_e16_fc1_pointer_table_raw(
    const void* stacked_expert_tokens,
    const int32_t* group_sizes,
    const int32_t* group_padded_offsets,
    const void* gate_w,
    const void* up_w,
    const void* packed_input,
    const void* gate_output,
    const void* up_output,
  ...
  if (expert < num_experts) {
    ...
    if (expert + 1 == num_experts && group_sizes[expert] < 128) {
      input_offset = static_cast<int64_t>(expert) * expert_block_m * hidden * 2;
      pointer_table[expert] = reinterpret_cast<int64_t>(packed_input) + input_offset;
    } else {
      pointer_table[expert] =
    ...
    pointer_table[num_experts + expert] =
    ...
    pointer_table[3 * num_experts + expert] =
        reinterpret_cast<int64_t>(up_w) + weight_offset;
    pointer_table[4 * num_experts + expert] =
...
"""
    ...
    def kernel(
        ...
        pointer_table: T.Tensor((5, num_experts), T.int64),
    ):
        with T.Kernel(1, threads=64) as _:
            T.import_source(source)
            T.call_extern(
                "handle",
                "build_e16_fc1_pointer_table_raw",
                ...
                gate_output.data,
                up_output.data,
                T.access_ptr(pointer_table[0, 0], "w", 5 * num_experts),
                num_experts,
                expert_block_m,
                hidden,
                intermediate,
...
def _get_kernels(
    ...
    if kernels is None:
        ...
        kernels = (
            ...
            (
                _build_e16_fc1_pointer_table_kernel(
                    hidden,
                    intermediate,
                    num_experts,
                    total_padded_tokens,
